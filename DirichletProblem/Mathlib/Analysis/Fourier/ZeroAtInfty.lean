...
@[simp]
theorem Lp.fourierTransformInvZeroAtInftyCLM_toLp_apply (f : 𝓢(V, E)) (x : V) :
    Lp.fourierTransformInvZeroAtInftyCLM V E (f.toLp 1) x = 𝓕⁻ f x := by
  calc
    _ = ((fourierTransformInvZeroAtInftyCLM V E) (toLpCLM ℂ (E := V) E 1 volume f)) x := by simp
    _ = (toZeroAtInftyCLM ℂ V E ∘L (fourierInvCLM ℂ 𝓢(V, E))) f x := by
      congr 1
      apply LinearMap.extendOfNorm_eq
      · apply SchwartzMap.denseRange_toLpCLM
        norm_num
      use 1
      simpa using norm_fourierInv_toBoundedContinuousFunction_le_toLp_one
    _ = 𝓕⁻ f x := by simp

@[simp]
theorem Lp.fourierTransformInvZeroAtInftyCLM_toBCF (f : Lp (α := V) E 1) :
    (Lp.fourierTransformInvZeroAtInftyCLM V E f).toBCF = Real.Lp.fourierTransformInv f := by
  apply (denseRange_toLpCLM (by norm_num)).induction_on
    (p := fun f ↦ (Lp.fourierTransformInvZeroAtInftyCLM V E f).toBCF = Lp.fourierTransformInv f)
    f (isClosed_eq (by fun_prop) (Real.Lp.fourierTransformInvCLM V E).cont)
  intro f
  ext x
  simpa [fourierInv_coe] using Real.fourierInv_congr_ae (coeFn_toLp f 1 volume).symm x

theorem Lp.fourierTransformInvZeroAtInftyCLM_apply_apply (f : Lp (α := V) E 1) (x : V) :
    Lp.fourierTransformInvZeroAtInftyCLM V E f x = 𝓕⁻ (f : V → E) x := by
  rw [← ZeroAtInftyContinuousMap.toBCF_apply, Lp.fourierTransformInvZeroAtInftyCLM_toBCF]
  simp
